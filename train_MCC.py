...
parser.add_argument('--src_dataset', type=str, default='shapenet', choices=['Syn', 'Kin', 'RS', 'modelnet', 'shapenet', 'scannet'])
...
parser.add_argument('--TransCls_on_src', type=str2bool, default=False, help='Using TransCls in source')
parser.add_argument('--TransCls_on_trgt', type=str2bool, default=False, help='Using TransCls in target')
...
parser.add_argument('--TransCls_weight', type=float, default=0.2, help='weight of the TransCls loss')
...
for epoch in range(args.epochs):
    ...
    for data1, data2 in zip(src_train_loader, trgt_train_loader):
        ...
        if data1 is not None:
            ...
            if args.Contrast_on_src:
                ...
                #ReSSL
                logitsq, ligitsk, logitso, feat_q, feat_k, probs_w, probs_s = model(src_data_t1, src_data_t2, src_data , device ,activate_C = True)

                loss_r = - torch.sum(
                    F.softmax(ligitsk.detach() / 0.04, dim=1) * F.log_softmax(logitsq / 0.1, dim=1), dim=1).mean()
                # NL
                loss_cs = nn.KLDivLoss(reduction='batchmean')(torch.log(probs_w), probs_s)

                x = epoch / float(args.epochs)
                w1 = x ** 2.2
                w2 = 0.3 * x ** 0.45
                w = min(w1,w2)

                loss_r = loss_r + 0.27 * w * loss_cs
                loss = args.Contrast_weight * loss_r
            ...
            if args.TransCls_on_src:
                ...
                src_logits,_ = model(src_data_t1, src_data_t2, src_data,activate_N = True)
        ...
        if data2 is not None:
            ...
            if args.Contrast_on_trgt:
                ...
                #ReSSL
                logitsq, ligitsk, logitso, feat_q, feat_k, probs_w, probs_s = model(trgt_data_t1, trgt_data_t2, trgt_data, device, activate_C = True)
            ...
            if args.TransCls_on_trgt:
                ...
                trgt_logits,_ = model(trgt_data_t1, trgt_data_t2, trgt_data,activate_N = True)
    ...
    if src_val_acc > src_best_val_acc:
        ...
        best_model = io.save_model(model)
        # best_model = io.save_modele(model)

        print("-----save best model-----")

    elif epoch >= 120 and epoch % 10 == 0:
        save_model = io.save_model_e(model,epoch)
        # save_model = io.save_model_ee(model,epoch)
